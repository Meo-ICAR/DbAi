...
Core Tables patients (aliased as p): The main patient registry. Contains demographics (datanascita, sesso_id), baseline comorbidities (diabete_id, ipertensione_id), and HIV history (dataHIV, CD4nadir). patient_visits (aliased as pv): Longitudinal follow-up data. Contains visit dates (visitadel), lab results (CD4, HIVRNA, Colesterolo, HDL, LDL), and carotid IMT measurements (Carotide_comune_sx, Bulbo_dx). plaches (aliased as pl): Details on carotid plaques (like stenosi, ecogenicita_id). Linked via pl.patient_visit_id = pv.id. dopplers (aliased as d): A separate, wide table for Doppler exam results. Linked via d.patientid = p.id. centers (aliased as c): Links centercode to the full center name.

Dictionary Tables (for Filtering) These tables define categories. To filter by a category, you must JOIN its dictionary table. Smoking (fumos on p.fumo_id): Values are 'si', 'no', 'ex'. Diabetes (diabetes on p.diabete_id): Values are 'si', 'no', 'non so'. Hypertension (ipertensiones on p.ipertensione_id): Values are 'si', 'no', 'pregresso', 'non so'.
Hepatitis (epatites on p.epatite_id): Values include 'si HBV', 'si HCV', 'si HCV e HBV', 'no'. Use LIKE '%HCV%' for HCV.
ART Status (artterapies on p.artterapia_id): Values are 'si' (on therapy), 'no', 'pregresso' (past therapy).
